...
- **いきなり画面は止まりません**。経験値が 78% を超えると HUD の下に
  「▸ まもなく LEVEL UP」が出て、満タンになると「LEVEL UP!」の予告帯と
  カウントダウンバーが 1.5 秒表示され、その間はスローモーになります。
...
画面は **1280×720（16:9）**。横15列・最大10行の広い盤面です。
...
- 5種類の画面サイズ（PC / 縦長 / iPhone横 / Android横）で**キャンバスが 16:9 のまま**
  はみ出さないこと
