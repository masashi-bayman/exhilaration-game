...
(function (global) {
  ...
  // iOS はサイレントスイッチが入っていると WebAudio が鳴らない。
  // 無音の <audio> を再生してオーディオセッションを「再生中」にすると回避できる。
  function unlockIOS() {
    var iOS = /iP(hone|ad|od)/.test(navigator.userAgent) ||
              (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
    if (!iOS || silentEl) return;
    try {
  ...
  var Sfx = {
    ...
    setMuted: function (m) {
      ...
      if (silentEl) {
        if (m) { try { silentEl.pause(); } catch (e) {} }
        else { try { silentEl.play(); } catch (e) {} }
      }
    },
